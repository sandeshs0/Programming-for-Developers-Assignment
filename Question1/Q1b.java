...
public class Q1b {
    public static int EngineBuildTime(int[] engineList, int splitCost) {
        int[] dp = new int[engineList.length + 1]; // minimum time to build i engines

        Arrays.fill(dp, Integer.MAX_VALUE); //filling integer infinity
        dp[0] = 0; // minimum time to build engine 0 as 0

        // Iterating through engines
        for (int i = 1; i <= engineList.length; i++) {
            // Calling the DP again
            dp[i] = engineList[i - 1] + splitCost; 
            // iterating through each possible split point
            for (int j = 1; j < i; j++) {
                dp[i] = Math.min(dp[i], dp[j] + dp[i - j]); // updating minimum time
            }
        }
        return dp[engineList.length]; //final time will be in dp of total engines
    }

    public static void main(String[] args) {
        int[] engines = {1,2,3};
        System.out.println("Minimum time to build all engines: " + EngineBuildTime(engines, 1));
    }
}